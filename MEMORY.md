...
| 2026-08-09 | PR #74 merged (squash `ea8f7f3d`): default LLM combo + unresolvable chat fallback. Ensure `default` (`["llmfree"]`) first trong combo list, chặn delete/rename, LLM chat fallback sang default khi model không resolve. Manifest 673/673 PASS, vitest 17/17 PASS, build PASS, smoke test `tmp` -> `minimaxai/minimax-m3`. |
| 2026-08-08 | PR #73 merged (squash `4bac2eac`): cascade disable model -> remove from combos + console log provider prefix. (1) Helper `removeModelsFromAllCombos(ids, providerAlias?)` trong `combosRepo.js` (atomic, exact-match) wire vào `POST /api/models/disabled`; 4/4 test PASS. (2) `LiveFallbackChain` ModelNode gate provider prefix qua `PROVIDER_IDS` (id+alias+aliases, 157 ids), text-only label, trailing slug; thêm `public/providers/tokenrouter.png` (128x128). 9 file +281/-3, 634/634 manifest PASS. |
| 2026-08-07 | Console log single-model provider prefix: ModelNode gate provider prefix trên PROVIDER_IDS.has(parts[0]) (import REGISTRY build Set id+alias+aliases, 157 ids). Text-only label (max-w 110px), không icon. Model hiển thị chỉ trailing slug (vd `nvidia/minimaxai/minimax-m3` -> `#5 nvidia minimax-m3`), `title={rawName}` tooltip full path. TokenRouter bổ sung icon `public/providers/tokenrouter.png` (3-node hub tím `#8B5CF6`, 128×128). 634/634 PASS, 24/24 test PASS. |
| 2026-08-07 | Cascade disable → combo: `removeModelsFromAllCombos(ids, providerAlias?)` trong `combosRepo.js` xóa exact-match ids khỏi mọi combo.models[] (atomic transaction, return count updated). Wire vào `POST /api/models/disabled`. 4/4 unit test PASS (combos-remove-models.test.js), manifest 634/634 PASS. Trigger từ NVIDIA EOL `deepseek-v4-pro`/`deepseek-v4-flash` (case Sếp yêu cầu 2026-08-07). |
