...
import { isCustomTool, unwrapCustomToolArguments } from "../../diepxuan/codex/customToolBridge.js";
...
function emitToolCall(state, emit, tc) {
  ...
  if (!state.funcItemAdded[tcIdx] && callId && state.funcNames[tcIdx]) {
    ...
    const custom = isCustomTool({ name: state.funcNames[tcIdx] });
    ...
    emit("response.output_item.added", {
      ...
      item: {
        ...
        type: custom ? "custom_tool_call" : RESPONSES_ITEM.FUNCTION_CALL,
  ...
  if (tc.function?.arguments) {
    ...
    if (state.funcItemAdded[tcIdx] && refCallId && !isCustomTool({ name: state.funcNames[tcIdx] })) {
...
function closeToolCall(state, emit, idx) {
  ...
  if (callId && !state.funcItemDone[idx]) {
    ...
    const custom = isCustomTool({ name: state.funcNames[idx] });
    ...
    emit("response.output_item.done", {
      ...
      item: {
        id: `${custom ? "ctc" : "fc"}_${callId}`,
        type: custom ? "custom_tool_call" : RESPONSES_ITEM.FUNCTION_CALL,
        ...(custom ? { input: unwrapCustomToolArguments(args) } : { arguments: args }),
        call_id: callId,
